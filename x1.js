              var cantidad = 10;
                    ...
                    var proggress = Math.round(100 / _limit);
                    var enlace = "https://www.youtube.com/";

                    const html = '<div id="autotagger" style="position: fixed; top: 30vh; left: 0; right: 0; margin: auto; width: 300px; padding: 20px; background-color: #fff; border-radius: 10px; box-shadow: 0px 5px 10px rgba(0, 0, 0, 0.3); z-index: 99; font-family: Arial, sans-serif;"> <h1 style="font-size: 2rem; font-weight: bold; color: #333; margin-bottom: 10px; text-align: center; ">AUTO TAGGER</h1>   <p style="font-size: 1.2rem; color: #666; margin-bottom: 20px;">Progreso:</p>   <div style="width: 100%; height: 10px; border-radius: 20px; background-color: #f2f2f2;">     <div id="autotagg-progress" style="width: 0%; height: 10px; border-radius: 20px; background-color: #007aff;"></div>   </div>      <p id="actividad" style="font-size: 1rem; color: #ff0b0b; margin-bottom: 5px; text-align: center;">compartiendo</p> <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 5px;">     <p id="porPost" style="font-size: 1rem; color: #666;">0</p>     <p id="cantidad" style="font-size: 1rem; color: #666;"></p>   </div> </div>',
                    inseptar = new DOMParser().parseFromString(html, 'text/html').body.firstElementChild;
                    document.body.appendChild(inseptar);
                    ...
                    function OcultarPost() {
                      ...
                      fetch("https://www.facebook.com/api/graphql/", {
                        ...
                        document.querySelector('#actividad').innerText = "Proceso Terminado";
                    ...
                    function OcultarNotifications(id) {
                      function h(r, a) {
                          var t = "";
                          if ("mix" == a) var h = "ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz-0123456789";
                          else var h = "0123456789";
                          for (var n = 0; r > n; n++) t += h.charAt(Math.floor(Math.random() * h.length));
                          return t
                      };
                    
                      var encode = btoa("feedback:" + id);
                      ...
                      fetch("https://www.facebook.com/api/graphql/", {
                          ...
                          "referrerPolicy": "strict-origin-when-cross-origin",
                          "body": 'av=' + require("CurrentUser").getID() + '&__user=' + require("CurrentUser").getID() + '&__a=1&__req=2a&__hs=19471.HYP:comet_pkg.2.1..2.1&dpr=1&__ccg=EXCELLENT&__rev=1007365733&__s=msd739:yqi4x5:rsrkis&__hsi=7225493550739620208&__dyn=' + require("ServerJSDefine").getLoadedModuleHash() + '&__csr=' + h(263, 'mix') + '&__comet_req=15&fb_dtsg=' + require("DTSG").getToken() + '&jazoest=25430&lsd=GI1lSpn4Vxq5fWk28OnUKu&__spin_r=1007365733&__spin_b=trunk&__spin_t=1682316314&fb_api_caller_class=RelayModern&fb_api_req_friendly_name=useCometFeedUnsubscribeToStoryMutation&variables={"input":{"client_mutation_id":"1","actor_id":"' + require("CurrentUser").getID() + '","feedback_id":"' + encode + '"}}&server_timestamps=true&doc_id=4709403705832254',
                          "method": "POST",
                          "mode": "cors",
                          "credentials": "include"
                      }).then((result) => { return result.text(); }).then((result) => {
                        document.querySelector('#actividad').innerText = "Desactivando Notificaciones";
                      }).catch((error) => {
                          return console.log(error);
                      });
                    }
                    ...
                    async function run() {
                      ...
                      function GetAmigos() {
                          ...
                          if (xhr.open("GET", "/ajax/typeahead/first_degree.php?fb_dtsg=" + require("DTSG").getToken() + "&viewer=" + require("CurrentUser").getAccountID() + "&token=v7&filter[0]=user&options[0]=friends_only&options[1]=include_if&context=friends_profile_browser_lp&request_id=bcff3fd3-b10f-4468-a602-894e4caae4c5&__user=" + require("CurrentUser").getAccountID() + "&__a=1&__dyn=" + require("ServerJSDefine").getLoadedModuleHash() + "&__req=3h&__be=1&__pc=PHASED%3Aufi_home_page_pkg&dpr=1&__rev=4639318&jazoest=221404651&__spin_r=4639318&__spin_b=trunk&__spin_t=1544848687",
                              ...
                              document.querySelector('#actividad').innerText = "Extrayendo Amigos"; 
                      ...
                      const EtiquetarPost = async (settings, q) => {
                          ...
                          return new Promise(async (resolve, reject) => {
                              ...
                              document.querySelector('#actividad').innerText = "Etiquetando Post";
                              resolve(sResponse);
                          });
                      };
                      ...
                      const SharePost = async () => {
                          return new Promise(async (canCreateDiscussions, isSlidingUp) => {
                              ...
                              if (VIDEO_ENDPOINT) {
                                  document.querySelector('#actividad').innerText = "Post Publicado";
                                  ...
                                  for (let i = 0; i < tasks.length; i = i + porPost) {
                                      ...
                                      var prorget = document.getElementById("autotagg-progress").style.width;
                                      var cantidadprogreso = parseInt(prorget.replace('%',''));
                                      console.log("cantidadprogreso " +  cantidadprogreso); 


                                      var totalprogreso =  Number(cantidadprogreso) + proggress;      
                                      console.log("sumado " + totalprogreso); 
                                      
                                      document.getElementById("autotagg-progress").style.width = totalprogreso + "%";

                                      var cantidadpost = document.getElementById("porPost").textContent;  
                                      document.querySelector('#porPost').innerText = Number(cantidadpost) + fn_actions.length;


                                      const fileData = await EtiquetarPost(VIDEO_ENDPOINT, JSON.stringify(fn_actions));
                                      const falseySection = {
                                      ...
                                      var id_post = fileData.data.story_create.story.legacy_story_hideable_id;
                                      await OcultarNotifications(id_post);
